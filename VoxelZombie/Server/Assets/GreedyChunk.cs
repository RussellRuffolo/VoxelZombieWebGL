...
{
    public IWorld world;
    private Voxel[] voxels = new Voxel[16 * 16 * 16 + 3];
    ...
    {
        return voxels[x * 16 * 16 + y * 16 + z + 3];
    }

    public void SetVoxel(int x, int y, int z, Voxel value)
    {
        voxels[x * 16 * 16 + y * 16 + z + 3] = value;
    }
    ...
    public void init()
    {
        ...
        chunkPosY = ID.Y * 8;
        chunkPosZ = ID.Z * 8;
        // Todo (Russell): Could you explain to me why we set the voxels based on ChunkID? I'm missing something core.
        voxels[0] = (Voxel)ID.X;
        voxels[1] = (Voxel)ID.Y;
        voxels[2] = (Voxel)ID.Z;
    }
    ...
    public void CreateMessage()
    {
        ...
        for (int i = 0; i < voxels.Length; i++)
        {
            chunkDataMessage.WriteByte((byte)voxels[i]);
        }
